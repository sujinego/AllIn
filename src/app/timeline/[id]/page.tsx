...
import type { Timeline, TimelineEntry, TimelineTask } from '@/types'
...
async function getTimelineData(id: string) {
  ...
  const [timelineRes, entriesRes, tasksRes, sessionRes] = await Promise.all([
    supabase
      .from('timelines')
      .select('*')
      .eq('id', id)
    ...
    supabase
      .from('timeline_tasks')
      .select('id, timeline_id, phase_id, title, start_date, end_date, memo')
      .eq('timeline_id', id)
      .order('start_date', { ascending: true }),
    supabase.auth.getSession(),
  ])

  if (!timelineRes.data) return { timeline: null, entries: [], tasks: [], userId: null }
  ...
  return {
    ...
    tasks: (tasksRes.data ?? []) as TimelineTask[],
...
export default async function TimelineDetailPage({ params }: Props) {
  ...
  const { timeline, entries, tasks, userId } = await getTimelineData(id)
